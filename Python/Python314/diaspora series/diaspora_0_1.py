...
CARS = {
    "The Apex": {
        "vulnerabilities": ["hotwire", "engine_tamper"],
        "value": 50000,
        "status": "secure",
    },
    "The Sentinel": {
        "vulnerabilities": ["electronic_jam", "remote_access"],
        "value": 75000,
        "status": "secure",
    },
    "The Wanderer": {
        "vulnerabilities": ["physical_ram", "tire_slash"],
        "value": 30000,
        "status": "secure",
    },
}
...
DEFENSES = {
    "Reinforced Doors": {
        "cost": 100,
        "effectiveness": {"hotwire": 3, "physical_ram": 1},
        "installed": False,
    },
    "Advanced Alarm System": {
        "cost": 150,
        "effectiveness": {"electronic_jam": 1, "smash_window": 3},
        "installed": False,
    },
    "Automated Oil Slick": {
        "cost": 200,
        "effectiveness": {"getaway_attempt": 2},
        "installed": False,
        "uses": 1,
    },
    "Remote Immobilizer": {
        "cost": 500,
        "effectiveness": {"getaway_attempt": 3, "hotwire": 2},
        "installed": False,
        "cooldown": 3,
    },
    "Guard Dog": {
        "cost": 500,
        "effectiveness": {"smash_window": 2, "physical_ram": 1},
        "installed": False,
    },
}
    ...
    while thief_progress < max_thief_progress and turns_taken < max_turns:
        ...
        for i, (def_name, data) in enumerate(DEFENSES.items()):
            if data["installed"]:
                ...
                if "cooldown" in data and defense_cooldowns[def_name] > 0:
                    cooldown_status = (
                        f" (Cooldown: {defense_cooldowns[def_name]} turns)"
                    )
                elif "uses" in data:
        ...
        try:
            ...
            if 0 <= choice_idx < len(available_actions):
                chosen_defense_name = (
                    available_actions[choice_idx].split(".")[1].split("(")[0].strip()
                )
                chosen_defense_data = DEFENSES[chosen_defense_name]

                if (
                    "cooldown" in chosen_defense_data
                    and defense_cooldowns[chosen_defense_name] > 0
                ):
                    print(
                ...
                if "cooldown" in chosen_defense_data:
                    defense_cooldowns[chosen_defense_name] = chosen_defense_data[
                        "cooldown"
                    ]
        ...
        defense_value = 0
        if chosen_defense_name and thief_approach in chosen_defense_data.get(
            "effectiveness", {}
        ):
        ...
        thief_power = (
            random.randint(1, 5) + (current_garage_level - 1) * 50
        )  # Thieves get tougher
...
if __name__ == "__main__":
    ...
    print(
        "In a city plagued by organized crime, your unique collection of vehicles is under threat."
    )
    ...
    while game_running:
        ...
        if game_running:  # If player chose to undertake a mission
            ...
            if (
                all(car["status"] == "secure" for car in CARS.values())
                and current_garage_level < 3
